...
_h, _s = ce.check(_pair("Проекты шли по-разному.", "Я запустил три проекта. Два окупились."))
check("«три проекта, два окупились» без источника: ошибка, не заметка", bool(_h), str((_h, _s)))
_h, _s = ce.check(_pair("Это важно.", "Во-первых, это один из ключевых пунктов. В первую очередь для новичков."))
check("идиомы «один из», «в первую очередь» не факты", not _h, str((_h, _s)))
_h, _s = ce.check(_pair("Задача сложная, но решение есть.",
                        "С одной стороны, задача сложная. С другой, решить её можно в два счёта."))
check("пара идиом с числительными не считается фактами", not _h, str((_h, _s)))
