...
SCANNER: dict[str, list[str | tuple[str, str]]] = {
    ...
    # прямым текстом. StoryScope (arXiv:2604.03136, COLM 2026), параллельный
    # корпус 61 608 историй: явная мораль у ИИ в 77% текстов против 52% у
    # людей. Кодом ловится только узкий лексический подкласс - шаблонные
    # формулы морали; остальная категория N нелексическая и оценивается
    # LLM-слоем аудита по каталогу, как «Фигуративный ноль». Шаблоны сужены,
    # чтобы не цеплять «мораль» как понятие этики и школьные «уроки истории»:
    # нужна связка мораль+история/вывод либо прямое «учит нас».
    "Финальная мораль": [
        ("мораль этой истории / мораль проста",
         r"\bмораль\s+(?:этой\s+|сей\s+)?(?:истори\w+|проста|такова)\b"),
        ("эта история учит нас",
         r"\bистори\w\s+(?:учит|напоминает)\s+(?:нас|нам)\b"),
        ("эта история о том, что",
         r"\bэта\s+истори\w\s+о\s+том,\s+что\b"),
    ],
...
GENRE_MUTED_BANS: dict[str, set[str]] = {
    "academic": {
        "Является", "Длинное тире", "Данный/Данная/Данное",
        "В условиях [прил.] [сущ.]", "Подводя итог / Таким образом",
        "В связи с этим",
    },
    "legal": {
        "Является", "Данный/Данная/Данное", "В условиях [прил.] [сущ.]",
        "В связи с этим", "Длинное тире",
    },
    "fiction": {
        "Длинное тире", "от X до Y (ложный диапазон)", "Не только X, но и Y",
        "Не просто X, а Y",
    },
}
