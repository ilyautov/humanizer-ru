...
1. Куда сдвигается текст. Скор говорит «стало лучше», но скор мерит расстояние
   ...
   полностью чистых людей 15%, а не 100%. Значит выход скилла надо измерять
...
27B измеряет способность gemma следовать инструкции на 25 тысяч токенов, а не сам
...
def report() -> int:
    ...
    if any(r["proxy"] for r in rows):
        print("\n[осторожно] прогоны с proxy=истина считались локальной моделью: они измеряют\n"
