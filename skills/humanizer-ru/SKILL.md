...
- **Конкретен.** Не «многие компании», а «три проекта». Не «эксперты считают», а «я попробовал, не сработало». Имена, числа, случаи.
...
НЕ трогай, если:
- **Триада осмысленная.** «Пришёл, увидел, победил» — это не правило трёх ради ритма, а три реальных события. Бей только искусственные триады синонимов («важный, значительный и ключевой»).
- **Тире авторское и единичное.** Один длинный тире на весь текст в сильной позиции — это стиль, а не маркер. Бей частоту (тире в каждом втором предложении), а не само существование. В неформальном тексте всё равно лучше дефис, но не выдумывай проблему там, где её нет.
- **Повтор намеренный.** Анафора, рефрен, повтор для нажима («Он не пришёл. Он не позвонил. Он просто исчез.») — это приём, а не синонимическая бедность.
...
- **Структура оправдана длиной.** В лонгриде на 3000 слов подзаголовки и списки нужны. Подтип «подзаголовок на каждые 2-3 предложения» — про КОРОТКИЙ текст, не про статью.
- **Формальный регистр — требование жанра.** Деловое письмо, оффер, документ имеют право быть формальными. Канцелярит убирай, но вежливые обороты и регистр не ломай.
- **Гладкость от мастерства, а не от AI.** Хороший редактор тоже выравнивает текст. Если перплексия низкая, но есть мнение, конкретика и характерный голос — это, скорее всего, человек. Низкая перплексия сама по себе не улика.
...
После: «Стэнфорд гонял эксперимент два года: удалёнщики на 13% продуктивнее. У себя в команде вижу примерно то же.»
...
- [ ] Ноль длинных тире «—» во всём тексте (только дефисы «-», запятые, двоеточия). Исключение — если пользователь явно разрешил длинное тире для проф. редактуры
