...
SCANNER: dict[str, list[str | tuple[str, str]]] = {
    ...
    "Почерк модели": [
        ...
        # Дефис и короткое тире в той же роли («Антитела - это»): модели Gemma и
        # Llama пишут так вместо длинного. Только внутри строки, иначе пункт
        # списка «- это» после заголовка стал бы определением.
        ("X — это Y", r"[а-яё»)]\s+—\s+это\s|[а-яё»)][ \t]+[–-][ \t]+это\s"),
