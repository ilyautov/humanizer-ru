...
# Между числом и существительным допускаются HTML-теги: карточки сайта набраны
# как «<b>54</b><span>признака…». Без этого гейт месяц не видел 54/20 на главной
# при каталоге 64/21 (нашёл внешний разбор витрины 2026-09-07).
_GAP = r"(?:(?:</?[a-z]+>)+\s*|\s+)"
# Число со знаком минус или частью другого числа не счётчик: на сайте лежит
# настоящий вывод сканера «-27  маркеры: 14», это штраф, а не размер каталога.
_NUM = r"(?<![-\d.,])(\d+)"
RE_PATTERN_COUNT = re.compile(rf"\b{_NUM}(?={_GAP}{PATTERN_NOUNS})")
...
RE_BAN_COUNT = re.compile(rf"\b{_NUM}(?={_GAP}{BAN_NOUNS})")
...
RE_CATEGORY_COUNT = re.compile(rf"\b{_NUM}(?={_GAP}{CATEGORY_NOUNS})")
