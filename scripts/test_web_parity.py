...
Лексическое разнообразие (MATTR) сверяется точным равенством: число словоформ
и сам MATTR до последнего бита. Токенизация там нарочно своя, без razdel, и
расхождение даже в одном слове сдвинуло бы штраф на пороге. Повтор фраз между
абзацами сверяется так же: список фраз и число просмотренных словоформ
дословно, заметки сканера целиком.
...
SYNTHETIC = {
    ...
    # Токенизация лексического разнообразия: регистр, ё/Ё, дефисы внутри и по
    # краям, латиница внутри слова, цифры, ударение (U+0301), неразрывный дефис.
    "lexical": ("ЁЛКА ёлка Елка. Кто-то КТО-ТО кто -то, -нибудь по-настоящему-то "
                "Wi-Fi-роутер roboтакси 5-й 2024-го за́мок замок северо\u2011запад "
                "«Цитата» — тире; слово-  -слово ") * 12,
    # Повтор фраз между абзацами: ё/е и регистр, запятые внутри цепочки, точка,
    # цифры и латиница рвут её, цитаты в трёх видах кавычек, заголовок
    # Markdown, n-грамма из служебных слов, повтор внутри одного абзаца.
    "repeats": ("Мы живём в мире, где всё меняется очень быстро, и надо успевать.\n"
                "## Мы живём в мире, где всё меняется очень быстро\n\n"
                "Работа требует внимания к деталям и постоянного обучения в IT-команде.\n"
                "«Работа требует внимания к деталям и постоянного обучения», сказал он. "
                "\"И в то же время как и было\" и „в то же время как и было“.\n"
                "Итак, МЫ ЖИВЕМ в мире, где все меняется очень быстро. Цена выросла в 2020 году "
                "почти вдвое для всех. Короче, работа требует внимания к деталям и постоянного "
                "обучения в Wi-Fi-зоне.\n\nЦена выросла в 2021 году почти вдвое для всех, "
                "и в то же время как и было, и в то же время как и было."),
...
def main() -> int:
    ...
    for genre in (None, "academic", "news"):
        ...
        for f in files:
            ...
            elif (rep.lexical.tokens, rep.lexical.mattr) != (w["lexical"]["tokens"], w["lexical"]["mattr"]):
                # Без допуска: токенизация и MATTR обязаны совпасть до бита,
                # иначе штраф за разнообразие разъедется на пороге.
                failures.append(f"{tag}: лексика py {rep.lexical} vs js {w['lexical']}")
            elif rep.repeats.as_dict() != w["repeats"]:
                failures.append(f"{tag}: повтор фраз py {rep.repeats.as_dict()} vs js {w['repeats']}")
            elif list(sc.notes) != w["notes"]:
                failures.append(f"{tag}: заметки py {sc.notes} vs js {w['notes']}")
    ...
    if failures:
        for msg in failures:
            print("  ✗", msg)
        return 1
    print("OK — браузерный сканер совпадает с Python по банам, маркерам, ритму, повторам фраз, заметкам и "
