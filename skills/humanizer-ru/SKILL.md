...
без жанра сканер ловит учёных чаще, чем нейросети. Запомни балл «было». Нет `razdel`
и `pymorphy3`, сканер упал или команд нет вовсе: скажи пользователю одной строкой,
что сканер не запустился и балла не будет (как поставить, написано в его ошибке и
в README), и работай вручную.
...
| Длинное тире «—», короткое «–» вне числовых диапазонов и дефис с пробелами в роли тире | Запятая, двоеточие, точка или перестройка. Тире между подлежащим и сказуемым («Дружба [тире] это связь») меняется на «это», «называется» или глагол, не на запятую: запятая делает сказуемое приложением. Убирается всегда; оставить тире можно только по явной просьбе пользователя |
...
> только оговорки мнения из группы A.
